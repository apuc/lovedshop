...
<div class="container">
    <form class="form-horizontal" name="edit_form" id="edit_form" method="post" action="/admin/attrvalue/update/{$model->id}">
        <div class="form-group">
            <label for="firstname">Атрибут:</label>
            <br>
            <select name="attr_id" id="attr_id" class="form-control" required="required">
                {foreach from=$attribute item=attr}
                    {if $attr->id == $model->attr_id}
